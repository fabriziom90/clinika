  ...
  <AuthenticatedLayout section="logs">
    ...
    <div class="mb-3 d-flex">
      <input
        ...
        placeholder="Cerca per azione o modello"
      />
      <Link
        :href="`/admin/audit-logs?search=${search}`"
        class="main-button"
      >
        Cerca
      </Link>
    ...
    {{ console.log(logs) }}
      ...
      <tbody>
        <tr v-for="log in filteredLogs" :key="log.id">
          ...
          <td><pre>{{ JSON.stringify(log.old_values, null, 2) }}</pre></td>
          <td><pre>{{ JSON.stringify(log.new_values, null, 2) }}</pre></td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="mt-3">
      <Link
        v-if="props.logs.prev_page_url"
        :href="props.logs.prev_page_url"
        class="btn btn-secondary me-2"
      >Prec</Link>
      <Link
        v-if="props.logs.next_page_url"
        :href="props.logs.next_page_url"
